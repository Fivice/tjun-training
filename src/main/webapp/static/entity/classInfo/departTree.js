...
$(function () {
    ...
    $('#organizerUnit').click(function (e) {
...
var setting = {
    ...
    callback: {
        ...
        onClick: function (e, treeId, treeNode, clickFlag) {
            ...
            /*if(node.node_flag != 1){*/
                $("#organizerUnit").val(nodes[0].name);
                $("#organizerUnitId").val(nodes[0].id)
            /*}*/
        },
...
function createTree2 () {
    var zNodes = [];
    $.ajax({
        url:ctx+'/classInfo/findDepartment.action',
        type:'get',
        dataType:'json',
    ...
    /*if(nodes[0].children !== undefined){*/
       /* zTree.selectNode(nodes[0].children[0]); //设置默认选中第一个子节点
        $("#organizerUnit").val(nodes[0].children[0].name);
        $("#organizerUnitId").val(nodes[0].children[0].id);*/
    /*}*/
    var f = $('#organizerUnitId').val();//所选的ID   例如 "12589"
    var treeObj = $.fn.zTree.getZTreeObj("haveclasstree");//ztree树的ID
