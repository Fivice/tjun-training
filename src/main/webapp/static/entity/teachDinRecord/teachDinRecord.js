...
// }
var teacherDingTable = function() {
    //初始化Table
    var oTable = new teacherDingTableInit();
    oTable.Init();
};
...
//     oTable.Init();
// });
var teacherDingTableInit = function () {
    var oTableInit = new Object();
    //初始化Table
    oTableInit.Init = function () {
        $('#table_teach_dining_record').bootstrapTable({
            url: 'findTeacherTable.action',  //请求后台的URL（*）
            method: 'get',                       	//请求方式（*）
            ...
            columns: [
                ...
                { field: 'Button', title: '详情', formatter: operateFormatter1}],//自定义方法，添加详情按钮
    ...
    oTableInit.queryParams = function (params) {
        var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            id:$("#id").val(),
            schoolName:$("#schoolName1").val(),
            classRoom:$("#classRoom1").val(),
...
function search() {
    ...
    $('#table_teach_dining_record').bootstrapTable('selectPage', 1);
}

function operateFormatter1 (value, row, index) {
    var result = "";
    result += "<button type=\"button\" class=\"btn btn-info\" onclick=\"EditViewById1('" + row.num + "','" + row.classId + "')\" title='查看详情'><i class=\"fa fa-chevron-circle-right\"></i></button>";
    return result;
} ;
...
//详情
function EditViewById1(num,classId){
    window.location.href=ctx +'/teachDining/form.action?num='+num+'&classId='+classId;
}
