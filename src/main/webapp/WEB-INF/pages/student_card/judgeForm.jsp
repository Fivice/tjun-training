...
<% String stuCardId = (String)request.getSession().getAttribute("id");
...
<head>
    ...
    <script src="../static/common/bootstrapvalidator/js/bootstrapValidator.js"></script>
...
<section class="panel">
    ...
    <form id="form1" action="${ctx}/studentCard/save.action" method="post"  style="overflow:hidden;">
        ...
        <div id="divQuestion">
            <fieldset class="fieldset" style="" id="fieldset1">
                ...
                <table cellspacing="0"  class="matrix-rating" style="">
                    ...
                    <div class="field-label">6.${type6}(如不满意请在下方“后勤服务方面”栏中详细说明)<span class="req">*</span><span class="qtypetip">&nbsp;</span></div>
                    <table cellspacing="0"  class="matrix-rating" style="">
                        <colgroup>
                            <col width="20%">
                            <col width="20%">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <div class="col-xs-12">
                                                <input type="text"  class="form-control"   id="reason${index.index }" name="result${index.index }" >
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
<script type="text/javascript">
    ...
    {
        var id = "<%=stuCardId%>";
        var result1 = $('input:radio[name="result24"]:checked').val();
        var result2 = $('input:radio[name="result25"]:checked').val();
        var result3= $('input:radio[name="result26"]:checked').val();
        var result4 = $('input:radio[name="result34"]:checked').val();
        var result5 = $('input:radio[name="result35"]:checked').val();

        if(result1 ==5|| result2 == 5 || result3 == 5 || result4 == 5 || result5 == 5){
            formValidator();
            $('#form1').data('bootstrapValidator').validate();//启用验证
            document.getElementById("reason29").focus();
            var flag = $('#form1').data('bootstrapValidator').isValid()//验证是否通过true/false
            console.log(flag)
            if(flag == true){
                console.log(id)
                $.ajax({
                    ...
                    dataType: 'json',
                    type: 'post',
                    url: "${ctx}/studentCard/save.action",
                    success: function (result) {
                        if (result.code == 1) {
                            ...
                            parent.layer.msg('评价成功!感谢您的评价！', {
                                ...
                                window.location.href = '${ctx}/studentCard/classAndStudentInfo.action?id=' + id;
                            });
                        } else {
    ...
    function formValidator() {
        $("#form1").bootstrapValidator({//根据自己的formid进行更改
