...
<section class="panel">
    <header class="panel-heading custom-tab navbar-fixed-top">
        <header class="panel-heading custom-tab navbar-fixed-top">
            <a href="#class-info" data-toggle="tab" aria-expanded="true">选择教师证号</a>
        </header>
    </header>
            ...
            <div class="tab-pane active" id="class-info">
                ...
                <div class="sign-up-info">
                    <div class="form-group">
                        <label class="col-xs-4 control-label">选择教师证号:</label>
                        <div class="col-xs-8">
                            <select class="form-control" id="id" name="number">
                                <option value="">选择教师证号</option>
                                <c:forEach items="${teacherCardList}" var="teacherCardList">
                                    <option value="${teacherCardList.number}">${teacherCardList.number}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function myfunction() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: {},
            url: ctx + '/teacherCard/form.action?number=' + number,
            success: function (result) {
                if (result.code == 1) {
                    window.location.href = ctx + '/teacherCard/form2.action?number=' + number;
                } else {
                    layer.msg('教师证失效!', {
                        icon: 2,
                        time: 1000
